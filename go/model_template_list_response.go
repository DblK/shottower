...
func AssertTemplateListResponseRequired(obj TemplateListResponse) error {
	...
	for name, el := range elements {
		if isZero := IsZeroValue(el); isZero {
			return &RequiredError{Schema: "Template List Response", Field: name}
		}
	}

	if err := AssertTemplateListResponseDataRequired(obj.Response); err != nil {
		return err
	}
	return nil
}
...
// Accepts only nested slice of TemplateListResponse (e.g. [][]TemplateListResponse), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseTemplateListResponseRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aTemplateListResponse, ok := obj.(TemplateListResponse)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertTemplateListResponseRequired(aTemplateListResponse)
	})
}

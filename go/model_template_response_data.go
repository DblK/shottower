...
func AssertTemplateResponseDataRequired(obj TemplateResponseData) error {
	...
	for name, el := range elements {
		if isZero := IsZeroValue(el); isZero {
			return &RequiredError{Schema: "Template Response Data", Field: name}
		}
	}
...
// Accepts only nested slice of TemplateResponseData (e.g. [][]TemplateResponseData), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseTemplateResponseDataRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aTemplateResponseData, ok := obj.(TemplateResponseData)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertTemplateResponseDataRequired(aTemplateResponseData)
	})
}

...
func AssertTemplateListResponseDataRequired(obj TemplateListResponseData) error {
	...
	for name, el := range elements {
		if isZero := IsZeroValue(el); isZero {
			return &RequiredError{Schema: "Template List Response Data", Field: name}
		}
	}

	for _, el := range obj.Templates {
		if err := AssertTemplateListResponseItemRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// Accepts only nested slice of TemplateListResponseData (e.g. [][]TemplateListResponseData), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseTemplateListResponseDataRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aTemplateListResponseData, ok := obj.(TemplateListResponseData)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertTemplateListResponseDataRequired(aTemplateListResponseData)
	})
}
